...
/**
 * @ORM\Entity(repositoryClass=TsocieteRepository::class)
 */
class Tsociete
{
    /**
    ...
    /**
     * @ORM\OneToMany(targetEntity=Tclient::class, mappedBy="clrSoc")
     */
    private $tclients;
    ...
    public function removeTclient(Tclient $tclient): self
    {
        if ($this->tclients->removeElement($tclient)) {
            ...
            if ($tclient->getClrSoc() === $this) {
                $tclient->setClrSoc(null);
            }
        }
    ...
    public function getXExt(): ?bool
    {
        return $this->xExt;
    }

    public function setXExt(bool $xExt): self
    {
        $this->xExt = $xExt;
